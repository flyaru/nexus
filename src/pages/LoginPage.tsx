...
export const LoginPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ email: 'admin@nexus.pro', password: 'demo123' })
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg)] p-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="card-surface w-full max-w-md rounded-2xl p-8 shadow-card">
        <h1 className="text-2xl font-semibold mb-2">NEXUS PRO</h1>
        <p className="text-sm text-[var(--muted)] mb-6">Admin / Agent Login</p>
        <form className="space-y-4" onSubmit={submit}>
          ...
          <p className="text-xs text-[var(--muted)]">Demo: admin@nexus.pro / demo123</p>
        </form>
      </div>
    </div>
  )
